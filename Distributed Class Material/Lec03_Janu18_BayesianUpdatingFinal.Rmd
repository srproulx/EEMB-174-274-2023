...
```{r}
binom_posterior_table =tibble(p_w_proposed = seq(from=0, to = 1, by = 0.01) ) 
```
```{r}
numT=9
numW=6

binom_posterior_table <- mutate(binom_posterior_table ,
                                likelihood=dbinom(numW,size=numT,prob=p_w_proposed),
                                prior=1/n(),
...
```{r}
ggplot(data=binom_posterior_table, aes(x=p_w_proposed, y=posterior))+
  geom_line(color="red")+
  geom_line(aes(x=p_w_proposed, y=prior) , color="blue")
```
## Make a different prior that incorporates the idea that parameters near 0 or 1 are really unlikely
...
binom_posterior_table <- mutate(binom_posterior_table ,
                                likelihood=dbinom(numW,size=numT,prob=p_w_proposed),
                                prior=1/n()* (1-2*abs(p_w_proposed-0.5) )^(1/2),
                                raw_posterior = likelihood * prior , 
                                posterior = raw_posterior/sum(raw_posterior))
```
